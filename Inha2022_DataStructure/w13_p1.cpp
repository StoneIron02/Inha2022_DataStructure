		...
		for (int i = 0; i < matrix[v->matrixId].size(); i++) {
			if (matrix[v->matrixId][i] != NULL) {
				...
				if (matrix[v->matrixId][i]->src == v) {
					max < matrix[v->matrixId][i]->dst->id ? max = matrix[v->matrixId][i]->dst->id : max = max;
				}
				else {
					max < matrix[v->matrixId][i]->src->id ? max = matrix[v->matrixId][i]->src->id : max = max;
				}
			}
		}
		if (!exist) {
